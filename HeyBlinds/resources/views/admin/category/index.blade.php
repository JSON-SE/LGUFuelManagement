    ...
    <div class="row pb-4">
        ...
        <div class="col-sm-6 pt-3 pt-lg-0 my-auto d-flex justify-content-sm-end">
            ...
            <a href="{{route('admin.category.create')}}" class="btn btn-primary d-flex align-items-center">
                ...
                <span class="d-none d-md-block">Create new category</span>
            </a>
        </div>
    </div>
                    ...
                    <table id="productaccordion" class="table table-hover ">
                        ...
                        <tbody id="sortable" class="product-option-list ui-sortable sortable">
                            @foreach($categories as $key => $category)
                                ...
                                <td><input type="checkbox" name="is_active" value="{{$category->id}}" {{$category->is_active == 1 ? 'checked' : ''}} onchange="showMenuActive({{$category->id}})" /></td>
                                    <td>
                                        <form class="d-flex" action="{{route('admin.category.destroy', $category->id)}}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <a href="{{route('frontend.category', $category->slug)}}" class="btn btn-sm text-primary" target="_blank">
                                            ...
                                            <a href="{{route('admin.category.edit', $category->slug)}}" class="btn btn-sm text-primary">
                                            ...
                                            <button class="btn btn-sm " onclick="return confirm('Are you sure you want to delete this item?');">
                                              ...
                                              <a href="{{route('admin.category.show.subcategory', $category->id)}}" class="btn btn-sm text-primary">
                                                Show Subcategory
                                            </a>
...
@section('scripts')
    <script>
        ...
        function showMenuActive(category_id){
           axios.post('/admin/category/menu-active',{
