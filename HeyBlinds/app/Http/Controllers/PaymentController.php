...
class PaymentController extends Controller
{
    ...
    public function generateOrderID($prefix){
        if(config('global.is_heyblinds_com') == true){
            $number = $prefix . '-' .mt_rand(1000000, 9999999); // better than rand()
        }
        else{
    ...
    function orderNumberExists($number) {
        return Order::whereOrderId($number)->exists();
    }
    /**
     ...
     * @return Array
     */
    public function paymentWithMonerisGetway($request, $orderPrice, $order_id, $customer_id)
    {
        // if (config('app.env') == 'local' || config('app.env')== 'development' || config('app.env')== 'staging' ) {
        //     $store_id = 'store1';
        //     $api_token = 'yesguy1';
        // }elseif (config('app.env') == 'production'){
        //     $store_id = 'gwca043206';
        //     $api_token = 'psW0i45AUzFGPWy7fg5o';
        // }
        $store_id = config('moneris.store_id');
        $api_token = config('moneris.api_token');

        $expire = explode("/", $request->card_expiry);
        $expiry_date = substr($expire[1], -2) . $expire[0];
        $card_number = preg_replace("/\s+/", "", $request->card_number);

        $card_value = $request->card_cvc;

        $dynamic_descriptor = "888-412-3439";

        $cvdTemplate = array(
            'cvd_indicator' => '1',
            'cvd_value' => $card_value
        );
        $string = $request->shipping_street;
        $shipping_street = intval(preg_replace('/[^0-9]+/', '', $string), 10);
        $avsTemplate = array(
         'avs_street_number'=>  $shipping_street,
         'avs_street_name' => $string,
         'avs_zipcode' => $request->shipping_postal_code,
         'avs_email' => $request->shipping_email,
         'avs_hostname'=> 'heyblinds.ca',
         'avs_browser' => $request->header('User-Agent'),
         'avs_shiptocountry' => 'CA',
         'avs_merchprodsku' => '',
         'avs_custip'=> request()->ip(),
         'avs_custphone' => $request->shipping_phone_number,
     );
    ...
    /**
     * Convert the payment response
     *
     * @param $paymemnt $a PaymentResponse
     *
     * @return Json
     */

    public function convertTheresponse($payment)
    {
        $paymentResult = [
            "CardType" => $payment->getCardType(),
            "TransAmount" => $payment->getTransAmount(),
            "TxnNumber" => $payment->getTxnNumber(),
            "ReceiptId" => $payment->getReceiptId(),
            "TransType" => $payment->getTransType(),
            "ReferenceNum" => $payment->getReferenceNum(),
            "ResponseCode" => $payment->getResponseCode(),
            "ISO" => $payment->getISO(),
            "Message" => $payment->getMessage(),
            "IsVisaDebit" => $payment->getIsVisaDebit(),
            "AuthCode" => $payment->getAuthCode(),
            "Complete" => $payment->getComplete(),
            "TransDate" => $payment->getTransDate(),
            "TransTime" => $payment->getTransTime(),
            "Ticket" => $payment->getTicket(),
            "TimedOut" => $payment->getTimedOut(),
            ...
            //'cvvResponseCode' => $payment->getCvdResultCode(),
        ];
